/* ─── Marriage & Union — Wedding customs and marriage law across 12,000 years ─── */
...
import {
  Heart, Scale, Handshake, Church, Gift, Shield, Flower2, Users,
  Search, ChevronDown, ChevronUp,
} from 'lucide-react'
import { MARRIAGES, MARRIAGE_CATEGORIES, ERA_LABELS, type Marriage } from '../data/marriage'

const CATEGORY_ICONS: Record<string, React.ElementType> = {
  ceremony: Heart, legal: Scale, arranged: Handshake, religious: Church,
  dowry: Gift, rights: Shield, customs: Flower2, modern: Users,
}
...
export default function MarriagePage() {
  ...
  const grouped = useMemo(() => {
    const map = new Map<string, Map<string, Marriage[]>>()
  ...
  const stats = useMemo(() => {
    ...
    for (const item of MARRIAGES) {
      byEra[item.era] = (byEra[item.era] || 0) + 1
      byCat[item.category] = (byCat[item.category] || 0) + 1
      civs.add(item.civilization)
    }
    return { total: MARRIAGES.length, byEra, byCat, civilizations: civs.size }
  }, [])

  return (
    <Box>
      <Breadcrumb items={[{ label: 'Marriage & Union' }]} />
      <Box mb={8} textAlign="center" py={8}
        bg="linear-gradient(135deg, #3D1F2E 0%, #6B2C4E 40%, #C53D6E 70%, #F687B3 100%)"
        borderRadius="2xl" border="1px solid #C53D6E">
        ...
        <Text fontFamily='"Cormorant Garamond", serif' fontSize={{ base: 'lg', md: 'xl' }}
          ...
          From clan exogamy to online dating — every ceremony, law, and custom
          that shaped how humans pair, partner, and build families.
        </Text>
        <Text fontFamily='"JetBrains Mono", monospace' fontSize="sm" color="#F687B3">
          {stats.total} milestones · {stats.civilizations} civilizations · 12,000 years
        </Text>
      </Box>
      ...
      <Flex justify="space-between" align="center" mb={4}>
        <Text fontFamily='"JetBrains Mono", monospace' fontSize="xs" color="#9E9A90">
          Showing {filtered.length} of {stats.total} marriage milestones
        </Text>
        {(selectedEra || selectedCategory || search) && (
        ...
        return (
          <Box key={era} mb={10}>
            ...
            {Array.from(catMap.entries()).map(([catId, items]) => {
              const catInfo = MARRIAGE_CATEGORIES.find(c => c.id === catId)
                    ...
                    {items.map(item => {
                      ...
                      return (
                        <Box key={item.slug} p={4} bg="white" border="1px solid #E4E2DC"
                          ...
                          _hover={{ shadow: 'md', borderColor: '#C53D6E' }}>
      ...
      <Box mb={8} p={6} bg="#3D1F2E" borderRadius="2xl">
        <Heading fontFamily='"Cinzel", serif' fontSize="xl" color="#F687B3" mb={4} textAlign="center">
          Turning Points in Marriage History
        </Heading>
        <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} gap={4}>
          {[
            { title: 'Property Creates Marriage', period: '~10,000 BCE', detail: 'With agriculture came land ownership, and with property came the need to control inheritance. Marriage likely originated as an economic contract — not a love story.', color: '#6B4D1B' },
            { title: 'Marriage as Sacrament', period: '1184 CE', detail: 'The Catholic Church declared marriage a permanent sacrament under divine authority. Divorce became impossible in Europe for 400 years — until Luther broke the monopoly.', color: '#8B4513' },
            { title: 'The Love Revolution', period: '~1800 CE', detail: 'For the first time in history, marrying for love became the norm in the West. Before this, arranged marriage was universal and love matches were considered dangerous.', color: '#A67C2E' },
            { title: 'Women Become Legal Persons', period: '1839 CE', detail: 'Married Women\'s Property Acts ended coverture — the doctrine that a wife had no legal existence separate from her husband.', color: '#C5963A' },
            { title: 'Marriage Equality', period: '2001 CE', detail: 'The Netherlands legalized same-sex marriage, starting the fastest expansion of marriage rights in history. 35 countries followed within 23 years.', color: '#4A90D9' },
            { title: 'Marriage Becomes Optional', period: '~2020 CE', detail: 'In many Western countries, fewer than half of adults are married. Cohabitation, single parenthood, and chosen family are redefining partnership beyond marriage.', color: '#6B3FA0' },
          ].map(m => (
            <Box key={m.title} p={4} bg="rgba(255,255,255,0.05)" borderRadius="lg" borderLeft="3px solid" borderLeftColor={m.color}>
              ...
              <Text fontFamily='"Inter", sans-serif' fontSize="xs" color="#FBB6CE" lineHeight={1.6}>{m.detail}</Text>
            </Box>
          ))}
        </SimpleGrid>
      </Box>

      <Box bg="#3D1F2E" borderRadius="2xl" p={8} textAlign="center" mb={8}>
        <Heading fontFamily='"Cinzel", serif' fontSize={{ base: 'lg', md: 'xl' }} color="#F687B3" mb={3}>
          "Marriage is the triumph of imagination over intelligence.<br />
          Second marriage is the triumph of hope over experience."
        </Heading>
        <Text fontFamily='"Inter", sans-serif' fontSize="sm" color="#E4E2DC" mb={2}>— Oscar Wilde</Text>
        <Text fontFamily='"JetBrains Mono", monospace' fontSize="xs" color="#9E9A90" mt={3}>
          {stats.total} marriage milestones documented · {stats.civilizations} civilizations represented · CC0 Public Domain
        </Text>
      </Box>
    </Box>
  )
