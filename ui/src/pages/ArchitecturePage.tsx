/* ─── Architecture of the World — Comprehensive History Page ─── */
...
import {
  Church, Landmark, Home, Shield, Pyramid, BrickWall, Factory,
  TreePine, Search, ChevronDown, ChevronUp,
} from 'lucide-react'
import { ARCHITECTURE, ARCHITECTURE_CATEGORIES, ERA_LABELS, type Architecture } from '../data/architecture'

const CATEGORY_ICONS: Record<string, React.ElementType> = {
  religious: Church, civic: Landmark, residential: Home, military: Shield,
  funerary: Pyramid, infrastructure: BrickWall, commercial: Factory, landscape: TreePine,
}
...
export default function ArchitecturePage() {
  ...
  const grouped = useMemo(() => {
    const map = new Map<string, Map<string, Architecture[]>>()
  ...
  const stats = useMemo(() => {
    ...
    return { total: ARCHITECTURE.length, byEra, byCat, civilizations: civs.size }
  }, [])

  return (
    <Box>
      <Breadcrumb items={[{ label: 'Architecture & Monuments' }]} />
      <Box mb={8} textAlign="center" py={8}
        bg="linear-gradient(135deg, #2D2A24 0%, #4A3728 40%, #96770B 70%, #D4AF37 100%)"
        ...
        <Text fontFamily='"Cormorant Garamond", serif' fontSize={{ base: 'lg', md: 'xl' }}
          ...
          A comprehensive registry of the buildings, monuments, and structures that
          defined civilizations — from Göbekli Tepe to the Burj Khalifa.
        </Text>
        <Text fontFamily='"JetBrains Mono", monospace' fontSize="sm" color="#D4AF37">
          {stats.total} structures · {stats.civilizations} civilizations · 11,500 years
        </Text>
      </Box>
      ...
      <Box mb={8} p={5} bg="#FAFAF8" border="1px solid #E4E2DC" borderRadius="xl">
        <Flex gap={4} flexWrap="wrap" align="center" mb={4}>
          <Box position="relative" flex={1} minW="200px">
            ...
            <Input placeholder="Search buildings, monuments, civilizations..."
              value={search}
      ...
      <Flex justify="space-between" align="center" mb={4}>
        <Text fontFamily='"JetBrains Mono", monospace' fontSize="xs" color="#9E9A90">
          Showing {filtered.length} of {stats.total} structures
        </Text>
        {(selectedEra || selectedCategory || search) && (
        ...
        return (
          <Box key={era} mb={10}>
            ...
            {Array.from(catMap.entries()).map(([catId, items]) => {
              const catInfo = ARCHITECTURE_CATEGORIES.find(c => c.id === catId)
              const CatIcon = CATEGORY_ICONS[catId] || Landmark
              return (
                <Box key={catId} mb={6}>
      ...
      <Box mb={8} p={6} bg="#082340" borderRadius="2xl">
        <Heading fontFamily='"Cinzel", serif' fontSize="xl" color="#D4AF37" mb={4} textAlign="center">
          The Arc of Architectural Innovation
        </Heading>
        <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} gap={4}>
          {[
            { title: 'Megaliths Before Farming', period: '~9500 BCE', detail: 'Göbekli Tepe\'s massive stone pillars prove that monumental architecture preceded agriculture — civilization may have begun with temples, not farms.', color: '#645E52' },
            { title: 'Egypt Builds for Eternity', period: '~2560 BCE', detail: 'The Great Pyramid stood as the tallest structure on Earth for 3,800 years. A civilization\'s architecture reveals what it valued most.', color: '#8B3A3A' },
            { title: 'Rome Engineers an Empire', period: '~100 BCE', detail: 'Concrete, arches, aqueducts, roads, and the Colosseum. Rome proved that infrastructure IS civilization.', color: '#96770B' },
            { title: 'Gothic Light & Height', period: '~1140 CE', detail: 'Ribbed vaults and flying buttresses turned stone walls into stained-glass windows. Medieval builders reached for heaven — literally.', color: '#D4AF37' },
            { title: 'Renaissance Harmony', period: '~1436 CE', detail: 'Brunelleschi\'s dome and Palladio\'s villas revived classical proportion. Architecture became a mathematical art form.', color: '#D69E2E' },
            { title: 'Iron & Steel Rise', period: '~1885 CE', detail: 'Chicago\'s steel-frame skyscrapers invented the modern city. The elevator made height profitable; steel made it possible.', color: '#4A90D9' },
            { title: 'Modernism Strips Away', period: '~1920 CE', detail: 'Bauhaus declared "less is more." Glass curtain walls, open plans, no ornament. Architecture for the machine age.', color: '#3182CE' },
            { title: 'Bilbao Effect', period: '1997 CE', detail: 'Gehry\'s Guggenheim proved a single building could revitalize a city. Architecture as economic strategy.', color: '#C53030' },
            { title: 'Building for the Planet', period: '2020s CE', detail: 'Net-zero energy, mass timber, vertical forests. The 21st century\'s challenge: build for 10 billion people without destroying the Earth.', color: '#6B3FA0' },
          ].map(m => (
      ...
      <Box bg="#2D2A24" borderRadius="2xl" p={8} textAlign="center" mb={8}>
        <Heading fontFamily='"Cinzel", serif' fontSize={{ base: 'lg', md: 'xl' }} color="#D4AF37" mb={3}>
          "Architecture is the learned game, correct and magnificent,<br />
          of forms assembled in the light."
        </Heading>
        <Text fontFamily='"Inter", sans-serif' fontSize="sm" color="#E4E2DC" mb={2}>— Le Corbusier</Text>
        <Text fontFamily='"JetBrains Mono", monospace' fontSize="xs" color="#9E9A90" mt={3}>
          {stats.total} structures documented · {stats.civilizations} civilizations represented · CC0 Public Domain
        </Text>
      </Box>
    </Box>
  )
