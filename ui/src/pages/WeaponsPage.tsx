/* ─── Weapons of the World — Comprehensive History Page ─── */
import React, { useState, useMemo } from 'react'
import { Box, Flex, Heading, Text, SimpleGrid, Input } from '@chakra-ui/react'
import { SectionHeading, StatCard } from '../components/DataCards'
import Breadcrumb from '../components/Breadcrumb'
import {
  Swords, Target, Shield, Ship, Crosshair, Flame,
  Plane, Zap, AlertTriangle, Search, Castle, ChevronDown, ChevronUp,
} from 'lucide-react'
import { WEAPONS, WEAPON_CATEGORIES, ERA_LABELS, type Weapon } from '../data/weapons'

const CATEGORY_ICONS: Record<string, React.ElementType> = {
  melee: Swords, ranged: Target, siege: Castle, gunpowder: Flame,
  naval: Ship, artillery: Crosshair, armor: Shield, aerial: Plane,
  modern: Zap, chemical: AlertTriangle,
}

export default function WeaponsPage() {
  const [search, setSearch] = useState('')
  const [selectedEra, setSelectedEra] = useState<string | null>(null)
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null)
  const [expandedWeapon, setExpandedWeapon] = useState<string | null>(null)

  const filtered = useMemo(() => {
    let w = WEAPONS
    if (selectedEra) w = w.filter(x => x.era === selectedEra)
    if (selectedCategory) w = w.filter(x => x.category === selectedCategory)
    if (search.trim()) {
      const q = search.toLowerCase()
      w = w.filter(x =>
        x.name.toLowerCase().includes(q) ||
        x.civilization.toLowerCase().includes(q) ||
        x.subcategory.toLowerCase().includes(q) ||
        x.origin.toLowerCase().includes(q) ||
        x.description.toLowerCase().includes(q)
      )
    }
    return w
  }, [selectedEra, selectedCategory, search])

  /* Group by era then category */
  const grouped = useMemo(() => {
    const map = new Map<string, Map<string, Weapon[]>>()
    for (const w of filtered) {
      if (!map.has(w.era)) map.set(w.era, new Map())
      const catMap = map.get(w.era)!
      if (!catMap.has(w.category)) catMap.set(w.category, [])
      catMap.get(w.category)!.push(w)
    }
    return map
  }, [filtered])

  const eraOrder = ['prehistoric', 'ancient', 'medieval', 'earlyModern', 'modern', 'contemporary']
  const stats = useMemo(() => {
    const byEra: Record<string, number> = {}
    const byCat: Record<string, number> = {}
    const civs = new Set<string>()
    for (const w of WEAPONS) {
      byEra[w.era] = (byEra[w.era] || 0) + 1
      byCat[w.category] = (byCat[w.category] || 0) + 1
      civs.add(w.civilization)
    }
    return { total: WEAPONS.length, byEra, byCat, civilizations: civs.size }
  }, [])

  return (
    <Box>
      <Breadcrumb items={[{ label: 'Arms & Warfare' }]} />
      {/* ─── Hero ─── */}
      <Box mb={8} textAlign="center" py={8}
        bg="linear-gradient(135deg, #2D2A24 0%, #524E44 40%, #8B3A3A 70%, #D4AF37 100%)"
        borderRadius="2xl" border="1px solid #D4AF37">
        <Heading fontFamily='"Cinzel", serif' fontSize={{ base: '2xl', md: '4xl' }} fontWeight={700}
          color="#FAFAF8" mb={3}>
          Weapons of the World
        </Heading>
        <Text fontFamily='"Cormorant Garamond", serif' fontSize={{ base: 'lg', md: 'xl' }}
          color="#E4E2DC" maxW="750px" mx="auto" lineHeight={1.8} mb={3}>
          A comprehensive registry of every significant weapon in human history —<br />
          from the first stone hand axe to hypersonic glide vehicles.
        </Text>
        <Text fontFamily='"JetBrains Mono", monospace' fontSize="sm" color="#D4AF37">
          {stats.total} weapons · {stats.civilizations} civilizations · 72,000 years
        </Text>
      </Box>

      {/* ─── Summary Stats ─── */}
      <SimpleGrid columns={{ base: 2, md: 3, lg: 6 }} gap={3} mb={8}>
        {eraOrder.map(era => {
          const info = ERA_LABELS[era]
          return (
            <StatCard key={era} value={String(stats.byEra[era] || 0)}
              label={info.label} color={info.color} />
          )
        })}
      </SimpleGrid>

      {/* ─── Filters ─── */}
      <Box mb={8} p={5} bg="#FAFAF8" border="1px solid #E4E2DC" borderRadius="xl">
        <Flex gap={4} flexWrap="wrap" align="center" mb={4}>
          <Box position="relative" flex={1} minW="200px">
            <Box position="absolute" left={3} top="50%" transform="translateY(-50%)" zIndex={1}>
              <Search size={16} color="#9E9A90" />
            </Box>
            <Input
              placeholder="Search weapons, civilizations, origins..."
              value={search}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSearch(e.target.value)}
              pl={9} size="sm" bg="white" border="1px solid #E4E2DC" borderRadius="lg"
              fontFamily='"Inter", sans-serif' fontSize="sm"
              _focus={{ borderColor: '#D4AF37', boxShadow: '0 0 0 1px #D4AF37' }}
            />
          </Box>
        </Flex>

        {/* Era filter pills */}
        <Flex gap={2} flexWrap="wrap" mb={3}>
          <Text fontFamily='"JetBrains Mono", monospace' fontSize="10px" color="#9E9A90"
            textTransform="uppercase" mr={2} lineHeight="28px">
            Era:
          </Text>
          {eraOrder.map(era => {
            const info = ERA_LABELS[era]
            const isActive = selectedEra === era
            return (
              <Box key={era} as="button" px={3} py={1} borderRadius="full" fontSize="xs"
                fontFamily='"Inter", sans-serif' fontWeight={600} cursor="pointer"
                bg={isActive ? info.color : 'white'} color={isActive ? 'white' : '#524E44'}
                border="1px solid" borderColor={isActive ? info.color : '#E4E2DC'}
                onClick={() => setSelectedEra(isActive ? null : era)}
                _hover={{ bg: isActive ? info.color : '#FDF8ED' }}>
                {info.label} ({stats.byEra[era] || 0})
              </Box>
            )
          })}
        </Flex>

        {/* Category filter pills */}
        <Flex gap={2} flexWrap="wrap">
          <Text fontFamily='"JetBrains Mono", monospace' fontSize="10px" color="#9E9A90"
            textTransform="uppercase" mr={2} lineHeight="28px">
            Type:
          </Text>
          {WEAPON_CATEGORIES.map(cat => {
            const isActive = selectedCategory === cat.id
            const Icon = CATEGORY_ICONS[cat.id] || Swords
            return (
              <Box key={cat.id} as="button" px={3} py={1} borderRadius="full" fontSize="xs"
                fontFamily='"Inter", sans-serif' fontWeight={600} cursor="pointer"
                bg={isActive ? cat.color : 'white'} color={isActive ? 'white' : '#524E44'}
                border="1px solid" borderColor={isActive ? cat.color : '#E4E2DC'}
                onClick={() => setSelectedCategory(isActive ? null : cat.id)}
                _hover={{ bg: isActive ? cat.color : '#FDF8ED' }}>
                <Flex align="center" gap={1}>
                  <Icon size={12} />
                  {cat.label} ({stats.byCat[cat.id] || 0})
                </Flex>
              </Box>
            )
          })}
        </Flex>
      </Box>

      {/* ─── Results Count ─── */}
      <Flex justify="space-between" align="center" mb={4}>
        <Text fontFamily='"JetBrains Mono", monospace' fontSize="xs" color="#9E9A90">
          Showing {filtered.length} of {stats.total} weapons
        </Text>
        {(selectedEra || selectedCategory || search) && (
          <Box as="button" fontSize="xs" fontFamily='"Inter", sans-serif' color="#4A90D9"
            cursor="pointer" onClick={() => { setSelectedEra(null); setSelectedCategory(null); setSearch('') }}
            _hover={{ textDecoration: 'underline' }}>
            Clear filters
          </Box>
        )}
      </Flex>

      {/* ─── Grouped Weapons Display ─── */}
      {eraOrder.filter(era => grouped.has(era)).map(era => {
        const eraInfo = ERA_LABELS[era]
        const catMap = grouped.get(era)!

        return (
          <Box key={era} mb={10}>
            {/* Era Header */}
            <Box mb={4} pb={2} borderBottom="3px solid" borderColor={eraInfo.color}>
              <Flex align="center" gap={3}>
                <Box w="8px" h="8px" borderRadius="full" bg={eraInfo.color} />
                <Heading fontFamily='"Cinzel", serif' fontSize="xl" fontWeight={700} color="#2D2A24">
                  {eraInfo.label}
                </Heading>
                <Text fontFamily='"JetBrains Mono", monospace' fontSize="xs" color="#9E9A90">
                  {eraInfo.period}
                </Text>
              </Flex>
            </Box>

            {Array.from(catMap.entries()).map(([catId, weapons]) => {
              const catInfo = WEAPON_CATEGORIES.find(c => c.id === catId)
              const CatIcon = CATEGORY_ICONS[catId] || Swords

              return (
                <Box key={catId} mb={6}>
                  <Flex align="center" gap={2} mb={3}>
                    <CatIcon size={16} color={catInfo?.color || '#524E44'} />
                    <Text fontFamily='"Inter", sans-serif' fontSize="sm" fontWeight={700}
                      color={catInfo?.color || '#524E44'}>
                      {catInfo?.label || catId}
                    </Text>
                    <Text fontFamily='"JetBrains Mono", monospace' fontSize="10px" color="#9E9A90">
                      ({weapons.length})
                    </Text>
                  </Flex>

                  <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} gap={3}>
                    {weapons.map(w => {
                      const isExpanded = expandedWeapon === w.slug
                      return (
                        <Box key={w.slug} p={4} bg="white" border="1px solid #E4E2DC"
                          borderRadius="xl" borderTop="3px solid" borderTopColor={eraInfo.color}
                          cursor="pointer" transition="all 0.15s"
                          onClick={() => setExpandedWeapon(isExpanded ? null : w.slug)}
                          _hover={{ shadow: 'md', borderColor: '#D4AF37' }}>
                          {/* Header */}
                          <Flex justify="space-between" align="flex-start" mb={2}>
                            <Box flex={1}>
                              <Text fontFamily='"Cormorant Garamond", serif' fontSize="md"
                                fontWeight={700} color="#2D2A24" lineHeight={1.3}>
                                {w.name}
                              </Text>
                              <Text fontFamily='"JetBrains Mono", monospace' fontSize="10px"
                                color="#9E9A90" mt={0.5}>
                                {w.subcategory}
                              </Text>
                            </Box>
                            {isExpanded ? <ChevronUp size={14} color="#9E9A90" /> : <ChevronDown size={14} color="#9E9A90" />}
                          </Flex>

                          {/* Quick info */}
                          <Flex gap={2} flexWrap="wrap" mb={2}>
                            <Text fontSize="10px" fontFamily='"JetBrains Mono", monospace'
                              bg="#FDF8ED" px={1.5} py={0.5} borderRadius="md" color="#524E44">
                              {w.yearIntroduced}
                            </Text>
                            <Text fontSize="10px" fontFamily='"JetBrains Mono", monospace'
                              bg="#E8F0FE" px={1.5} py={0.5} borderRadius="md" color="#082340">
                              {w.civilization}
                            </Text>
                            <Text fontSize="10px" fontFamily='"JetBrains Mono", monospace'
                              bg="#F0FFF4" px={1.5} py={0.5} borderRadius="md" color="#22543D">
                              {w.origin}
                            </Text>
                          </Flex>

                          {/* Expanded details */}
                          {isExpanded && (
                            <Box mt={3} pt={3} borderTop="1px solid #E4E2DC">
                              <Text fontFamily='"Inter", sans-serif' fontSize="sm" color="#524E44"
                                lineHeight={1.7} mb={3}>
                                {w.description}
                              </Text>
                              <Box bg="#FDF8ED" p={3} borderRadius="lg" border="1px solid #E4E2DC">
                                <Text fontFamily='"JetBrains Mono", monospace' fontSize="10px"
                                  color="#D4AF37" textTransform="uppercase" mb={1}>
                                  Historical Impact
                                </Text>
                                <Text fontFamily='"Inter", sans-serif' fontSize="sm" color="#2D2A24"
                                  lineHeight={1.6} fontWeight={500}>
                                  {w.impact}
                                </Text>
                              </Box>
                            </Box>
                          )}
                        </Box>
                      )
                    })}
                  </SimpleGrid>
                </Box>
              )
            })}
          </Box>
        )
      })}

      {/* ─── Timeline of Military Innovation ─── */}
      <Box mb={8} p={6} bg="#082340" borderRadius="2xl">
        <Heading fontFamily='"Cinzel", serif' fontSize="xl" color="#D4AF37" mb={4} textAlign="center">
          The Arc of Military Innovation
        </Heading>
        <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} gap={4}>
          {[
            { title: 'The Stone Age Revolution', period: '~400,000 BCE', detail: 'Fire-hardened spears and hand axes gave Homo sapiens dominion over every other species.', color: '#645E52' },
            { title: 'Bronze Transforms Warfare', period: '~3300 BCE', detail: 'Metal weapons created professional armies, chariots, and the first empires in Mesopotamia and Egypt.', color: '#8B3A3A' },
            { title: 'Iron Democratizes Combat', period: '~1200 BCE', detail: 'Cheap, abundant iron armed citizen-soldiers: Greek hoplites, Roman legions, Celtic warriors.', color: '#96770B' },
            { title: 'The Stirrup Revolution', period: '~400 CE', detail: 'Stirrups allowed mounted warriors to fight with lances at full gallop, creating feudal cavalry dominance.', color: '#D4AF37' },
            { title: 'Gunpowder Changes Everything', period: '~1300 CE', detail: 'Chinese invention traveled via the Mongols to Europe. Cannons destroyed castles; muskets killed knights.', color: '#D69E2E' },
            { title: 'Industrial Mass Killing', period: '~1860 CE', detail: 'Rifled barrels, machine guns, railroads, and telegraph turned warfare into industrialized slaughter.', color: '#4A90D9' },
            { title: 'Total War From the Air', period: '~1940 CE', detail: 'Strategic bombing, aircraft carriers, and blitzkrieg made entire nations the battlefield.', color: '#3182CE' },
            { title: 'The Nuclear Threshold', period: '1945 CE', detail: 'Humanity gained the power to destroy itself. MAD doctrine made great-power war unthinkable.', color: '#C53030' },
            { title: 'Precision & Autonomy', period: '2000s CE', detail: 'Drones, guided missiles, cyber weapons, and AI-enabled targeting redefine what a "weapon" means.', color: '#6B3FA0' },
          ].map(m => (
            <Box key={m.title} p={4} bg="rgba(255,255,255,0.05)" borderRadius="lg" borderLeft="3px solid" borderLeftColor={m.color}>
              <Text fontFamily='"JetBrains Mono", monospace' fontSize="10px" color={m.color} mb={1}>
                {m.period}
              </Text>
              <Text fontFamily='"Cormorant Garamond", serif' fontSize="md" fontWeight={700} color="#FAFAF8" mb={1}>
                {m.title}
              </Text>
              <Text fontFamily='"Inter", sans-serif' fontSize="xs" color="#B8D4FE" lineHeight={1.6}>
                {m.detail}
              </Text>
            </Box>
          ))}
        </SimpleGrid>
      </Box>

      {/* ─── Closing ─── */}
      <Box bg="#2D2A24" borderRadius="2xl" p={8} textAlign="center" mb={8}>
        <Heading fontFamily='"Cinzel", serif' fontSize={{ base: 'lg', md: 'xl' }} color="#D4AF37" mb={3}>
          "I know not with what weapons World War III will be fought,<br />
          but World War IV will be fought with sticks and stones."
        </Heading>
        <Text fontFamily='"Inter", sans-serif' fontSize="sm" color="#E4E2DC" mb={2}>
          — Albert Einstein
        </Text>
        <Text fontFamily='"JetBrains Mono", monospace' fontSize="xs" color="#9E9A90" mt={3}>
          {stats.total} weapons documented · {stats.civilizations} civilizations represented · CC0 Public Domain
        </Text>
      </Box>
    </Box>
  )
}
